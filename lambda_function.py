...
def get_constituents(alreadyInvestedStocks):
  ...
  for st in alreadyInvestedStocks:
      if st not in sp500: # if not in there anyways
          sp500s.append(st)
  # need to add stocks we already invested in
  return sp500s

def get_data(stocks,alreadyInvestedStocks):
  data = yf.download(stocks.tolist(),period="1d",interval="1m")
  ...
  selection = mean_returns.mean().sort_values(ascending = False)[:20].index.to_list() # top 20 returns
...
def run(ALPACAUSER,ALPACAPW,GETNEWCONSTITUENTS = False):
    api = tradeapi.REST(
        # TRADINGBOT1
        ALPACAUSER,
        ALPACAPW,
    ...
    if clock.is_open: # if markets open, else cancel
      ...
      for stock in openorders:
        ...
        alreadyInvested = np.delete(alreadyInvested, np.where(constituents == stock))
...
def lambda_handler(event, context):
    try:
        GETNEWCONSTITUENTS = bool(str(os.environ['GETNEWCONSTITUENTS']))
        ALPACAUSER = str(os.environ['ALPACAUSER'])
        ALPACAPW = str(os.environ['ALPACAPW'])
        investments = run(ALPACAUSER,ALPACAPW,False)
        if investments == "Markets not open":
          return {
            'statusCode': 200,
            'message':'Markets not open.'}
        else:
          return {
              'statusCode': 200,
              'message':'Success! Invested: %s'%investments}
    except Exception as e:
        raise # for debugging
        return {
            'statusCode': 400,
